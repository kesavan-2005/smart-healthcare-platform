...
export default function Dashboard({ user, onLogout }) {
  ...
  const getApiBase = () => window.location.hostname === 'localhost' ? 'http://localhost:5000' : 'https://smart-healthcare-platform-2.onrender.com';
  ...
  const fetchSuggestions = async (symptomsArray) => {
    let res;
    try {
      res = await fetch(`${getApiBase()}/suggest`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ symptoms: symptomsArray })
      });
    } catch(err) {
      res = await fetch('http://localhost:5000/suggest', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ symptoms: symptomsArray })
      });
